...
def paired_paths(dest_nb_path: Path) -> tuple[Path, Path]:
    """Return (py_path, md_path) for a copied notebook path in notebooks/"""
...
def process_notebook(raw_nb: Path) -> tuple[Path, bool]:
    ...
    # --- Write python file (percent format) ---
    new_py_content = jupytext.writes(nb, fmt="py:percent")
    ...
    # --- Write markdown file ---
    new_md_content = jupytext.writes(nb, fmt="md")
